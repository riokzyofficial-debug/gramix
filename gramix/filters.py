...
class SupergroupChatFilter(BaseFilter):
    def check(self, update: Message) -> bool:
        from gramix.types.chat import ChatType
        return update.chat.type == ChatType.SUPERGROUP

class ChannelChatFilter(BaseFilter):
    def check(self, update: Message) -> bool:
        return update.chat.is_channel
...
class F:
    ...
    supergroup = SupergroupChatFilter()
    channel = ChannelChatFilter()
    poll = HasPollFilter()
    quiz = HasQuizFilter()
